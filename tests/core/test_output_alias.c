...
void test_output_alias_clone() {
  ...
  // validate new Alias Output object
  // validate amount
  ...
  // validate alias ID
  TEST_ASSERT_EQUAL_MEMORY(output->alias_id, new_output->alias_id, ADDRESS_ALIAS_BYTES);

  // validate state controller
  TEST_ASSERT_EQUAL_MEMORY(output->st_ctl, new_output->st_ctl, sizeof(address_t));

  // validate governance controller
  TEST_ASSERT_EQUAL_MEMORY(output->gov_ctl, new_output->gov_ctl, sizeof(address_t));
  ...
  TEST_ASSERT_EQUAL_MEMORY(output->state_metadata->data, new_output->state_metadata->data, output->state_metadata->len);
