...
void test_utxo_input() {
  ...
  TEST_ASSERT(utxo_inputs_add(&inputs, 0, NULL, 2, NULL, NULL) == -1);

  // test for -1 if index more than max_output_count
  TEST_ASSERT(utxo_inputs_add(&inputs, 0, tx_id0, UINT8_MAX, NULL, NULL) == -1);

  // test for unknown input type
  TEST_ASSERT(utxo_inputs_add(&inputs, 1, tx_id0, 2, NULL, NULL) == -1);

  // add tx_id0 and index 1
  TEST_ASSERT(utxo_inputs_add(&inputs, 0, tx_id0, 1, NULL, NULL) == 0);

  // Check if count of inputs is 1
  TEST_ASSERT_EQUAL_UINT16(1, utxo_inputs_count(inputs));

  // add two more tx IDs
  TEST_ASSERT(utxo_inputs_add(&inputs, 0, tx_id1, 2, NULL, NULL) == 0);
  TEST_ASSERT(utxo_inputs_add(&inputs, 0, tx_id2, 3, NULL, NULL) == 0);
  ...
  TEST_ASSERT(utxo_inputs_add(&inputs, 0, tx_id1, 4, NULL, NULL) == 0);

  // trying to add a new txn_id with output index that is present in list
  TEST_ASSERT(utxo_inputs_add(&inputs, 0, tx_id3, 2, NULL, NULL) == 0);

  // trying to add txn_id and output index that is present in the list in the same input
  TEST_ASSERT(utxo_inputs_add(&inputs, 0, tx_id2, 3, NULL, NULL) == -1);
...
void test_utxo_input_syntactic() {
  ...
  TEST_ASSERT(utxo_inputs_add(&inputs, 0, tx_id0, 0, NULL, NULL) == 0);
  TEST_ASSERT(utxo_inputs_add(&inputs, 0, tx_id0, 0, NULL, NULL) == -1);
