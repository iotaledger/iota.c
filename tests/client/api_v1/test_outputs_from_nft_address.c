...
void test_deserialize_outputs() {
  ...
  TEST_ASSERT(deserialize_outputs_from_nft_address(data_empty, res) == 0);
  ...
  TEST_ASSERT_NOT_NULL(res);
  TEST_ASSERT(deserialize_outputs_from_nft_address(data_1, res) == 0);
  TEST_ASSERT(res->is_error == false);
  TEST_ASSERT(res->u.output_ids->max_results == 1000);
...
void test_deserialize_outputs_err() {
  ...
  res_outputs_nft_address_t* res = res_outputs_nft_address_new();
  TEST_ASSERT_NOT_NULL(res);
  int ret = deserialize_outputs_from_nft_address(small_addr_err_400, res);
  TEST_ASSERT(ret == 0);
  ...
  res = res_outputs_nft_address_new();
  TEST_ASSERT_NOT_NULL(res);
  ret = deserialize_outputs_from_nft_address(big_addr_err_400, res);
  TEST_ASSERT(ret == 0);
  TEST_ASSERT(res->is_error == true);
  TEST_ASSERT_EQUAL_STRING(res->u.error->code, "400");
  TEST_ASSERT_EQUAL_STRING(res->u.error->msg,
