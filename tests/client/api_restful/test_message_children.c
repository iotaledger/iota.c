...
void test_deser_block_children() {
  ...
  res_block_children_t* ch = res_block_children_new();
  TEST_ASSERT_NOT_NULL(ch);

  int ret = deser_block_children(json_res, ch);
  TEST_ASSERT_EQUAL_INT(0, ret);
  TEST_ASSERT_FALSE(ch->is_error);
  TEST_ASSERT(ch->u.data->count == 4);
  TEST_ASSERT(ch->u.data->max_results == 1000);
  TEST_ASSERT_EQUAL_STRING("a30d95e791d9cefa52156051974d5396d48b517bd16644bf2d3d0f67e9f7d82f", ch->u.data->blk_id);
  TEST_ASSERT_EQUAL_INT(ch->u.data->count, res_block_children_len(ch));
  TEST_ASSERT_EQUAL_STRING("38c01ac57bef2407ba38900c0d78b1d7a0e51d78feb08fff14606024f5077048",
                           res_block_children_get(ch, 0));
  TEST_ASSERT_EQUAL_STRING("f63d38f0e10b65234824bbeb0fd0917b890089a8cc9cc2ddef22879b1785309d",
                           res_block_children_get(ch, 3));
  res_block_children_free(ch);
}
...
void test_get_msg_children() {
  ...
  TEST_ASSERT(get_block_children(&ctx, msg_id, ch) == 0);
  if (ch->is_error) {
    TEST_ASSERT_NOT_NULL(ch->u.error);
    print_block_children(ch, 0);
  } else {
    TEST_ASSERT_NOT_NULL(ch->u.data);
    size_t ch_num = res_block_children_len(ch);
...
int main() {
  ...
  RUN_TEST(test_deser_block_children);
