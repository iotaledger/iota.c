...
void test_deser_tx_message() {
  ...
  TEST_ASSERT_EQUAL_INT(4, msg_meta_parents_len(meta));
  ...
  TEST_ASSERT_EQUAL_STRING("ee6a7fae8287c718c22d6304ebc8ee6f5cc5622be1d177f7c7733a9e7e8cf408",
                           msg_meta_parent_get(meta, 3));
  TEST_ASSERT_NULL(msg_meta_parent_get(meta, 4));
...
void test_get_msg_meta() {
  ...
  // Milestone payload
  char const* const id_str = "c8ad52b30fdc4a510240e6dfe3794cfd51261ae7a394591ec2c1ba5bc80f28f5";
  // Transaction payload
  ...
  if (meta->is_error) {
    ...
    size_t parents = msg_meta_parents_len(meta);
    printf("%zu parents:\n", parents);
    for (size_t i = 0; i < parents; i++) {
      printf("\t%s\n", msg_meta_parent_get(meta, i));
    }
