...
static int connect_socket(http_client_config_t const* const config) {
  ...
  LOG_DBG("Preparing HTTPS GET request for %s:%s\n", config->url, port_text);

  // get address info from host
  hints.ai_family = AF_INET;
  hints.ai_socktype = SOCK_STREAM;
  st = getaddrinfo(config->url, port_text, &hints, &addr);
  if (st != 0) {
  ...
  if (sock >= 0) {
    ...
    ret = setsockopt(sock, SOL_TLS, TLS_HOSTNAME, config->url, strlen(config->url));
    if (ret < 0) {
  ...
  if (ret < 0) {
    LOG_ERR("Connect to %s failed (%d)", config->url, -errno);
    freeaddrinfo(addr);
    return -errno;
  }
  freeaddrinfo(addr);
