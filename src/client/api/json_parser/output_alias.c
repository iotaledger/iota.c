...
int json_output_alias_deserialize(cJSON *output_obj, transaction_essence_t *essence) {
  ...
  // create alias output
  output_alias_t *output = output_alias_new(amount, tokens, alias_id, state_index, state_metadata->data,
                                            state_metadata->len, foundry_counter, cond_blocks, feat_blocks);
  if (!output) {
