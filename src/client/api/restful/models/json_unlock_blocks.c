...
int json_unlock_blocks_deserialize(cJSON *blocks_obj, transaction_payload_t *payload_tx) {
  ...
  cJSON_ArrayForEach(elm, blocks_obj) {
    ...
    if (block_type->valueint == UNLOCK_BLOCK_TYPE_SIGNATURE) {  // signature unlock block
        ...
        if (cJSON_IsNumber(sig_type)) {
          if (sig_type->valueint == ADDRESS_TYPE_ED25519) {
            ...
            if (cJSON_IsString(pub) && cJSON_IsString(sig)) {
              ...
              memcpy(sig_block + 1 + UNLOCK_BLOCKS_PUB_KEY_HEX_STR_LEN, sig->valuestring,
                     UNLOCK_BLOCKS_SIGN_HEX_STR_LEN);
              if (unlock_blocks_add_signature(&payload_tx->unlock_blocks, sig_block,
                                              UNLOCK_BLOCKS_SIGN_BLOCK_STR_LEN) != 0) {
    ...
    } else if (block_type->valueint == UNLOCK_BLOCK_TYPE_REFERENCE) {  // reference unlock block
    ...
    } else if (block_type->valueint == UNLOCK_BLOCK_TYPE_ALIAS) {  // alias unlock block
      cJSON *ref = cJSON_GetObjectItemCaseSensitive(elm, JSON_KEY_REFERENCE);
      if (cJSON_IsNumber(ref)) {
    ...
    } else if (block_type->valueint == UNLOCK_BLOCK_TYPE_NFT) {  // NFT unlock block
      cJSON *ref = cJSON_GetObjectItemCaseSensitive(elm, JSON_KEY_REFERENCE);
      if (cJSON_IsNumber(ref)) {
