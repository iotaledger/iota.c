...
#include "client/api/restful/get_outputs_from_alias_address.h"
#include "client/api/json_utils.h"
#include "client/network/http.h"
#include "core/address.h"
#include "core/utils/iota_str.h"

static get_outputs_alias_address_t *outputs_new() {
  get_outputs_alias_address_t *ids = malloc(sizeof(get_outputs_alias_address_t));
  if (ids) {
    ids->count = 0;
    ids->max_results = 0;
...
static void outputs_free(get_outputs_alias_address_t *ids) {
  if (ids) {
    if (ids->outputs) {
...
res_outputs_alias_address_t *res_outputs_alias_address_new() {
  res_outputs_alias_address_t *res = malloc(sizeof(res_outputs_alias_address_t));
  if (res) {
    res->is_error = false;
...
int get_outputs_from_alias_address(iota_client_conf_t const *conf, char const addr[],
                                   res_outputs_alias_address_t *res) {
