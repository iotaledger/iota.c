...
#include "client/api/v1/get_transaction_included_message.h"
...
int get_transaction_included_message_by_id(iota_client_conf_t const *conf, char const tx_id[], res_message_t *res) {
  ...
  int snprintf_ret = snprintf(cmd_buffer, sizeof(cmd_buffer), "/api/v1/transactions/%s/included-message", tx_id);
...
int deser_get_transaction_included_message(char const *const j_str, res_message_t *res) {
  ...
  if (!res->is_error && res->u.msg->type != MSG_PAYLOAD_TRANSACTION) {
    return -1;
  }
