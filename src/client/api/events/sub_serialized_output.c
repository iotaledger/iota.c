...
int event_sub_tx_blk_tagged_data(event_client_handle_t client, int *mid, byte_t tag[], uint8_t tag_len, int qos) {
  ...
  // 36 is the max length for string blocks/transaction/tagged-data/0x, max hex-encoded-tag is 128
...
int event_sub_blk_tagged_data(event_client_handle_t client, int *mid, byte_t tag[], uint8_t tag_len, int qos) {
  ...
  // 24 is the max length for string blocks/tagged-data/0x, max hex-encoded-tag is 128
  ...
  sprintf(topic_buff, "blocks/tagged-data/0x%s", tag_str);
