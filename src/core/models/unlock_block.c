...
size_t unlock_blocks_serialize(unlock_list_t* blocks, byte_t buf[]) {
  ...
  LL_FOREACH(blocks, elm) {
    if (elm->block.type == UNLOCK_BLOCK_TYPE_SIGNATURE) {  // signature block
      ...
      memcpy(offset, &elm->block.type, sizeof(byte_t));
      offset += sizeof(byte_t);
      unlock_referential_data_t* block_data = elm->block.block_data;
      memcpy(offset, &block_data->index, sizeof(block_data->index));
      offset += sizeof(block_data->index);
    }
  }
  ...
  if (blocks) {
    ...
    LL_FOREACH(blocks, elm) {
      if (elm->block.type == UNLOCK_BLOCK_TYPE_SIGNATURE) {  // signature block
        ...
        printf("ref: %" PRIu16 " ]\n", ((unlock_referential_data_t*)elm->block.block_data)->index);
