...
retcode_t iota_client_were_addresses_spent_from(iota_client_service_t const* const service,
  ...
  result = service->serializer.vtable.were_addresses_spent_from_serialize_request(req, req_buff);
  if (result != RC_OK) {
    goto done;
  }
  ...
  result = service->serializer.vtable.were_addresses_spent_from_deserialize_response(res_buff->data, res);
