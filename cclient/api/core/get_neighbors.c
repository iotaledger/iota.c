...
retcode_t iota_client_get_neighbors(iota_client_service_t const* const service, get_neighbors_res_t* const res) {
  ...
  result = service->serializer.vtable.get_neighbors_serialize_request(req_buff);
  if (result != RC_OK) {
    goto done;
  }
  ...
  result = service->serializer.vtable.get_neighbors_deserialize_response(res_buff->data, res);
